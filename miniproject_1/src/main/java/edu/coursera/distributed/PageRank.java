...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRank = sites.join(ranks).flatMapToPair(kv -> {
            int websiteId = kv._1();
            ...
            Iterator<Integer> nbrEdgeItr = edge.edgeIterator();
            while (nbrEdgeItr.hasNext()) {
                int nbrEdge = nbrEdgeItr.next();
                contribs.add(new Tuple2<>(nbrEdge, currentRank / (double) edge.getNEdges()));
            }
        ...
        return newRank.reduceByKey( (r1, r2) -> r1 + r2).mapValues(v -> 0.15 + 0.85*v);
