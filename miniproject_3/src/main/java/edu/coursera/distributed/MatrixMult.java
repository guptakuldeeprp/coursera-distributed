...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int size = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD); // total no. of ranks
        ...
        endRow = endRow > nRows ? nRows : endRow;

        // For simplicity, broadcast all of matrices a & b to all ranks
        // A more efficient approach would be to only send the relevant rows/columns to each rank
        ...
        if (myRank == 0) {
            MPI.MPI_Request[] requests = new MPI.MPI_Request[size - 1]; // receive requests from other ranks
            for (int i = 1; i < size; i++) {
                final int rankStartRow = i * rowChunk;
                int rankEndRow = (i + 1) * rowChunk;
                rankEndRow = rankEndRow > nRows ? nRows : rankEndRow;
            ...
            // ok to be blocking since each rank send one set of data
